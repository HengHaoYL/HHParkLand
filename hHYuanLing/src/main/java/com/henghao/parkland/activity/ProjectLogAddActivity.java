 ...
 * 施工日志添加界面
 */
public class ProjectLogAddActivity extends ActivityFragmentSupport {
     ...
     * 项目内容
     */
    @ViewInject(R.id.et_projrect_content)
    private TextView et_projrect_content;
    ...
    private boolean checkData() {
        ...
        if (ToolsKit.isEmpty(et_projrect_content.getText().toString().trim())) {
            msg("项目内容不能为空");
            return false;
        }
    ...
    public void uploadUserHeader(List<File> photo, File document) {
        ...
        String content = et_projrect_content.getText().toString().trim();//项目内容
        ...
        multipartBilder.type(MultipartBuilder.FORM)
                ...
                .addFormDataPart("projectDetail", content)
            ...
            public void onResponse(Response response) throws IOException {
                ...
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            mActivityFragmentView.viewLoading(View.GONE);
                            Toast.makeText(ProjectLogAddActivity.this, result, Toast.LENGTH_SHORT).show();
                        }
                    });
                    finish();
